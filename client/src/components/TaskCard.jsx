        ...
        <div
          ...
          className={`bg-slate-800 p-4 mb-3 rounded-xl border ${
            snapshot.isDragging
              ...
              : "border-slate-700 hover:border-slate-600 shadow-sm"
          ...
          <div className="flex justify-between items-start mb-2">
            <div className="flex flex-wrap gap-1">
              {task.tags.map((tag, i) => (
                <span
                  key={i}
                  className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-md bg-${tag.color}-500/10 text-${tag.color}-400 border border-${tag.color}-500/20`}
                >
                  {tag.name}
                </span>
              ))}
