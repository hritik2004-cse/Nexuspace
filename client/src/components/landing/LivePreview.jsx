...
export default function LivePreview() {
  ...
  useEffect(() => {
    ...
    const runSequence = () => {
      ...
      const t1 = setTimeout(() => {
        setIsTyping(false);
        setMessages([{ id: 1, user: "Alex", text: "Just pushed the new Edge router configs.", color: "indigo" }]);
      }, 1500);
      ...
      const t3 = setTimeout(() => {
        setIsTyping(false);
        setMessages(prev => [...prev, { id: 2, user: "Sarah", text: "Deploying verifying clusters now. Moving ticket.", color: "emerald" }]);
      }, 4000);
      ...
      const tLoop = setTimeout(() => {
         if(cycleCount < 10) runSequence(); // Prevent infinite headless run
         cycleCount++;
      }, 9000);

      return () => { clearTimeout(t1); clearTimeout(t2); clearTimeout(t3); clearTimeout(t4); clearTimeout(tLoop); };
    };
  ...
  return (
    <section className="relative z-10 py-32 bg-[#030014] border-y border-white/5 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8 max-w-5xl mx-auto">
           {/* Mock Chat Feed */}
           <div className="flex-1 border border-white/10 rounded-2xl bg-slate-950/80 backdrop-blur-xl p-6 shadow-2xl relative overflow-hidden h-[400px] flex flex-col">
               ...
               <AnimatePresence>
                 {messages.map(msg => (
                   ...
                   >
                     ...
                     <div className="bg-white/5 border border-white/5 rounded-2xl rounded-tl-sm p-3 text-sm text-slate-300 relative group pointer-events-auto">
           ...
           {/* Mock Kanban Feed */}
           <div className="flex-1 w-full lg:w-96 border border-white/10 rounded-2xl bg-slate-950/80 backdrop-blur-xl p-6 shadow-2xl relative overflow-hidden h-[400px]">
             <h3 className="font-bold text-white mb-6 flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-emerald-400" /> Live Kanban View</h3>
             <div className="space-y-4 relative h-full">
               
               <div className="absolute inset-0 right-1/2 border-r border-white/10 border-dashed pointer-events-none"></div>
               ...
               <motion.div 
                 ...
                 className="w-[calc(50%-10px)] bg-slate-900 border border-white/10 rounded-xl p-4 shadow-xl z-20 relative cursor-grab active:cursor-grabbing hover:border-white/20 transition-colors"
               >
                 <div className="flex gap-1 mb-3">
                   <div className="w-8 h-2 rounded-full bg-amber-500/50"></div>
                   <div className="w-4 h-2 rounded-full bg-fuchsia-500/50"></div>
                 </div>
                 <h4 className="text-white text-sm font-bold mb-2">Configure Edge Routing</h4>
